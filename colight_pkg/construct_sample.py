...
class ConstructSample:
    ...
    def load_data_for_system(self, folder):
        ...
        # ---------------------------------------------------------
        # FIX: Ensure dummy phase 0 exists in the expanded PHASE map
        # ---------------------------------------------------------
        sim = self.dic_traffic_env_conf["SIMULATOR_TYPE"]
        phase_map = self.dic_traffic_env_conf["PHASE"][sim]

        if 0 not in phase_map:
            lane_count = len(next(iter(phase_map.values())))
            phase_map[0] = [0] * lane_count
            print("Inserted dummy phase 0 with length", lane_count)
    ...
    def construct_state(self,features,time,i):
        ...
        if self.dic_traffic_env_conf["BINARY_PHASE_EXPANSION"]:
            ...
            for key, value in state["state"].items():
                if key in features:
                    if "cur_phase" in key:
                        # print("DEBUG PHASE lookup:",
                        #     "SIM_TYPE =", self.dic_traffic_env_conf["SIMULATOR_TYPE"],
                        #     "PHASE keys =", self.dic_traffic_env_conf["PHASE"][self.dic_traffic_env_conf["SIMULATOR_TYPE"]].keys(),
                        #     "cur_phase value[0] =", value[0])
    ...
    def judge_action(self, time, i):
        action = self.logging_data_list_per_gen[i][time]['action']

        # If action is -1, treat it as "no-op" or "stay in current phase"
        if action == -1:
            # Option A: map -1 to a valid phase index (e.g., 0)
            return 0

            # Option B: or return the previous action
            # if time > 0:
            #     return self.logging_data_list_per_gen[i][time - 1]['action']
            # else:
            #     return 0
    ...
    def dump_sample(self, samples, folder):
        if folder == "":
            ...
            round_dir = self.path_to_samples
            with open(os.path.join(round_dir, "total_samples_{0}.pkl".format(folder)), "ab+") as f:
                pickle.dump(samples, f, -1)
        else:
