...
class CoLightAgent(Agent): 
    def __init__(self, 
        ...
        dic_path=None, 
        cnt_round=None, 
        best_round=None, bar_round=None,intersection_id="0", DIC_PHASE_MAP= None):
        """
        ...
        self.DIC_PHASE_MAP = DIC_PHASE_MAP
        #self.len_feature=13
        ...
        self.num_actions = len(self.DIC_PHASE_MAP) 
        ...
        # Treat None as round 0 (generator process)
        if cnt_round is None or cnt_round == 0:
            # initialization
...
"""
    ...
    def choose_action(self, count, state):
        ...
        # act may be numpy array or nested list → flatten it
        act = np.array(act).reshape(-1).astype(int).tolist()
    ...
    # def choose_action(self, count, state):
    #     q_values, attention = self.action_att_predict([state])
    #     q_values = q_values[0]          # shape: (num_actions,)
    #     action = int(np.argmax(q_values))
    #     return action, attention[0]
    
    # def choose_action(self, count, state):
    #     # Forward pass
    #     q_values, attention = self.action_att_predict([state])
    #     q_values = q_values[0]      # shape: [num_agents, num_actions]

    #     # Select best action per agent
    #     actions = np.argmax(q_values, axis=1)   # shape: [num_agents]

    #     return actions, attention[0]
